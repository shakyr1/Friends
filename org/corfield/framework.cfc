...
*/
	...
	public string function buildURL( string action = '.', string path = variables.magicBaseURL, any queryString = '' ) {
		...
		return URLSessionFormat(basePath);
	}
    ...
    public string function getEnvironment() {
        if ( findNoCase( "apps", CGI.SERVER_NAME ) ) return "prod";
		else if ( findNoCase( "localhost", CGI.SERVER_NAME ) ) return "local";
		else return "dev";
    }
	...
	public void function redirect( string action, string preserve = 'none', string append = 'none', string path = variables.magicBaseURL, string queryString = '', string statusCode = '302' ) {
		...
		location( URLSessionFormat(targetURL), false, statusCode );
	}
	...
	private void function setupFrameworkDefaults() {
        ...
        // Added by BARISHD to track Root Path
		if ( !structKeyExists(variables.framework, 'baseapplicationpath') ) {
			variables.framework.baseapplicationpath = '';
		}
	...
	private void function viewNotFound() {
		...
		private boolean function checkdatasource(string datasource) {
		try{				
			factory = CreateObject("java","coldfusion.server.ServiceFactory");
			dsService = factory.DatasourceService;
			isDatasource = dsService.verifyDataSource(datasource);
			return true;		
		}
		catch(exception e){
			return false;	
	...
	private any function getEncryptionKeys(string datasource) {
		qSessionKey = queryExecute("select login_key, application_key from session_keys",
       		      {datasource="cf_daniel"}
		);
		return qSessionKey;
	}


	public string function encodeurl(string plainstr) {         
/*
	    var vKey = '';

        if(checkdatasource('cf_daniel')){
            vKey = '9ERwyfZ8kNlvQbneQ/DcCg==';
        }
        else{
            vKey = '9ERwyfZ8kNlvQbneQ/DcCg==';
        }
*/		 
		var vKey = '9ERwyfZ8kNlvQbneQ/DcCg==';
		var encryptionmethod = 'AES';	
		var decryptEncoding = 'Base64';		
	    var cfencode = encrypt(plainstr, vKey, encryptionmethod, decryptEncoding);
	    
		return cfencode;		
	}
    
	public string function decodeurl(string encstr) {         
		var vKey = '9ERwyfZ8kNlvQbneQ/DcCg=='; 
		var encryptionmethod = 'AES';	
		var decryptEncoding = 'Base64';		
	    var estr = REReplace(encstr,' ','+','ALL');
	    var cfencode = decrypt(estr, vKey, encryptionmethod, decryptEncoding);
	    
		return cfencode;		
	}
	
	public void function queryinsert(struct columns, string table_name, string datasource) {         	 	

	    var dataObj = createObject('component', 'vendor.org.corfield.dbinsert');      
        rc.result = dataObj.insert(columns, table_name , datasource);               
